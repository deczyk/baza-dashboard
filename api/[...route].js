...
async function handleTataboardCalendar(req, res) {
  ...
  const calendars = lists.flatMap(({ account, data: list }) => (list.items || []).filter((item) => !String(item.id).includes('#holiday') && !String(item.id).includes('#contacts')).map((item) => ({ key: `${account.id}:${item.id}`, accountId: account.id, id: item.id, summary: item.summary || 'Bez nazwy' })));
  const selectedCalendars = Array.isArray(saved.selectedCalendars) && saved.selectedCalendars.length ? saved.selectedCalendars : calendars.filter((item) => item.id === 'primary').map((item) => item.key);
