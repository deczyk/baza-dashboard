...
async function handleTataboardCalendar(req, res) {
  ...
  const saved = data.tataboardCalendar || {};
  const accounts = Array.isArray(saved.accounts) ? saved.accounts : (saved.refreshToken ? [{ id: 'konto-1', refreshToken: saved.refreshToken, calendarIds: saved.calendarIds || [saved.calendarId || 'primary'] }] : []);
  if (!accounts.length) { send(res, 200, { connected: false, accounts: [], events: [], calendars: [], selectedCalendars: [] }); return; }
  const authenticated = await Promise.all(accounts.map(async (account) => {
    const response = await fetch('https://oauth2.googleapis.com/token', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ client_id: process.env.GOOGLE_CLIENT_ID, client_secret: process.env.GOOGLE_CLIENT_SECRET, refresh_token: account.refreshToken, grant_type: 'refresh_token' }) });
    const token = await response.json(); if (!token.access_token) throw new Error('Jedno z połączeń Google wygasło. Połącz je ponownie.');
    return { ...account, google: (path, options = {}) => werboardGoogle(token.access_token, path, options) };
  }));
  const lists = await Promise.all(authenticated.map(async (account) => ({ account, data: await account.google('/users/me/calendarList?minAccessRole=writer') })));
  const calendars = lists.flatMap(({ account, data: list }) => (list.items || []).filter((item) => !String(item.id).includes('#holiday') && !String(item.id).includes('#contacts')).map((item) => ({ key: `${account.id}:${item.id}`, accountId: account.id, id: item.id, primary: Boolean(item.primary), summary: item.summary || 'Bez nazwy' })));
  const validSavedSelection = Array.isArray(saved.selectedCalendars) ? saved.selectedCalendars.filter((key) => calendars.some((item) => item.key === key)) : [];
  const selectedCalendars = validSavedSelection.length ? validSavedSelection : (calendars.filter((item) => item.primary).map((item) => item.key).concat(calendars.length && !calendars.some((item) => item.primary) ? [calendars[0].key] : []));
  if (req.method === 'POST' && action === 'set-calendars') { const selected = Array.isArray(req.body.calendarKeys) ? req.body.calendarKeys.filter((key) => calendars.some((item) => item.key === key)) : []; await bazaStore.mutateRecord((state) => { state.tataboardCalendar = { ...(state.tataboardCalendar || {}), accounts, selectedCalendars: selected }; }); send(res, 200, { ok: true, selectedCalendars: selected }); return; }
  if (req.method === 'POST' && action === 'create') { const title = String(req.body.title || '').trim(); const date = String(req.body.date || ''); const calendarKey = String(req.body.calendarKey || selectedCalendars[0] || ''); const target = calendars.find((item) => item.key === calendarKey); if (!title || !date || !target) { send(res, 400, { error: 'Podaj nazwę, datę i wybierz kalendarz.' }); return; } const account = authenticated.find((item) => item.id === target.accountId); const event = await account.google(`/calendars/${encodeURIComponent(target.id)}/events`, { method: 'POST', body: JSON.stringify({ summary: title, start: { date }, end: { date: new Date(new Date(`${date}T12:00:00`).getTime() + 86400000).toISOString().slice(0, 10) } }) }); send(res, 200, { ok: true, event: werboardEvent(event) }); return; }
  const eventLists = await Promise.all(selectedCalendars.map(async (key) => { const target = calendars.find((item) => item.key === key); if (!target) return []; const account = authenticated.find((item) => item.id === target.accountId); const result = await account.google(`/calendars/${encodeURIComponent(target.id)}/events?timeMin=${encodeURIComponent(new Date().toISOString())}&timeMax=${encodeURIComponent(new Date(Date.now() + 30 * 86400000).toISOString())}&singleEvents=true&orderBy=startTime&maxResults=30`); return (result.items || []).map((event) => ({ ...werboardEvent(event), calendarKey: target.key, calendarName: target.summary })); }));
